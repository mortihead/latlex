...
procedure Time;
 ...
 function ExitFromTest : boolean;
  ...
  HideMouse;
   eX := WhereX; eY := WhereY;
       { окно стало на 3 строки выше (10 -> 13): пункты Yes/No всегда
         занимают 3 строки (пустая-пункт-пустая, даже без видимой рамки
         и без тени - у безрамочного меню её больше нет), плюс пустая
         строка-отступ после вопроса. SubMenu(29,10,...) - координаты
         кнопочного окна абсолютные (не относительно диалога), 29
         центрирует его по ширине, 10 - первая свободная внутренняя
         строка после отступа. }
       MakeWindow(W[1],18,7,61,13,True,True,True, 127, 127, 127,
			  ' the Lation Lexicon ');
...
function SortWords : byte;
 ...
 var Sa, Sb, Sc : integer;
  ...
  begin
    HideMouse;
     sX := WhereX; sY := WhereY;
       { см. комментарий у ExitFromTest - окно выше (11 -> 14: +1 строка
         отступа после текста, +3 строки под Yes/No). }
       MakeWindow(W[1],15,7,65,14,True,True,True, 127, 127, 127,
			  ' the Lation Lexicon ');
       if not DisplayWindow(W[1]) then ShowError(2);
       {$R-}
